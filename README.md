...
- 运气分从简单线性公式升级为基于真实五星概率分布的统计型评分

**算法重构**

- **旧公式（已废弃）**：`运气分 = (90 - 平均出金) / 80 × 100`
  - 问题：未考虑软保底、概率非线性、样本量等因素

- **新模型**：基于原神实际抽卡概率分布
  - 角色/常驻/集录池：基础 0.6%，第74抽起软保底(+6%/抽)，第90抽硬保底
  - 武器池：基础 0.7%，第63抽起软保底(+7%/抽)，第80抽硬保底
  - 单次运气分 = P(X > n) × 100（比你更非的玩家比例，分数越高运气越好）
  - 总体运气分 = 所有单次运气分的算术平均

**新增功能**

- **单次运气分**：每个完整五星间隔对应一个评分
- **总体运气分**：各池独立计算，综合运气按样本数加权
- **统计可信度**：根据有效五星样本数量显示等级
  - 1~3 次：数据较少
  - 4~9 次：参考
  - 10~19 次：较可靠
  - 20+ 次：可靠
- **9级运气评级**：极欧/非常欧/比较欧/略欧/正常/略非/比较非/非常非/极非

**UI 改动（最小化）**

- 首页运气环下方文案从「高于 XX% 旅行者」改为「理论百分位 XX%」
- 新增「可信度」显示
- 无五星数据时显示「暂无数据」

**架构调整**

- 新增 `GachaProbabilityModel.kt`：独立概率模型（PMF/CDF/运气分计算）
- 新增 `LuckConfidence` 枚举 + `LuckVerdict` 对象
- `PoolStats` 新增 `luckScore`/`singleLuckScores`/`luckConfidence` 字段
- `GachaReport` 新增 `overallLuckScore`/`overallLuckConfidence` 字段
- 运气分计算全部集中在分析层，UI 层仅展示结果

**单元测试**

- 新增 `GachaProbabilityModelTest.kt`
- 覆盖：概率模型基础、各抽数运气分、边界值、单调性、可信度等级、评级边界、不同池配置

**影响文件**

- 新增：`GachaProbabilityModel.kt`、`GachaProbabilityModelTest.kt`
- 修改：`GachaStats.kt`、`GachaStatsCalculator.kt`、`HomeViewModel.kt`、`HomeScreen.kt`
