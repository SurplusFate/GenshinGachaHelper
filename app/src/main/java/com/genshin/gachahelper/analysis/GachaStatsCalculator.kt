...
class GachaStatsCalculator @Inject constructor() {
    ...
    fun calculatePoolStats(
        ...
        val sortedRecords = records.sortedByDescending { it.orderNumber }
        ...
        // UP 率计算
        val upFiveStars = fiveStarRecords.count { it.itemName in upItems }
    ...
    fun calculateCurrentPity(records: List<GachaRecordEntity>): Int {
        val sorted = records.sortedByDescending { it.orderNumber }
    ...
    fun calculateFiveStarIntervals(records: List<GachaRecordEntity>): List<Int> {
        val sorted = records.sortedBy { it.orderNumber } // 正序，最老在前
    ...
    fun generateReport(
        ...
        characterUpItems: List<String> = emptyList()
    ): GachaReport {
        val characterStats = if (characterRecords.isNotEmpty()) {
            calculatePoolStats(characterRecords, GachaType.CHARACTER.value, characterUpItems)
        } else null
        ...
        val allRecords = characterRecords + weaponRecords + standardRecords
